  ...
  async _processTIFF(tiff) {
    ...
    if (!this.options.bounds) {
      ...
      if (window.Worker && this.options.useWorker) {
        const worker_src =
          "onmessage = function(e){let data = e.data.data; let noDataValue = e.data.noDataValue; let min = data.filter(val=> val !== noDataValue).reduce((a,b)=>Math.min(a,b)); let max = data.filter(val => val !== noDataValue).reduce((a,b)=>Math.max(a,b)); postMessage({min:min, max:max});}";
        ...
        this.min = this.raster.data[0].reduce((a, b) =>
          b === this.options.noDataValue ? a : Math.min(a, b)
        );
        this.max = this.raster.data[0].reduce((a, b) =>
