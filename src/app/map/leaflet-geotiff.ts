...
const LeafletGeotiff = L.ImageOverlay.extend({
  options: {
    arrayBuffer: null,
    arrowSize: 20,
    band: 0,
  ...
  initialize(url: any, options: any) {
    this._url = url;
    this.raster = {};
  ...
  async _getData() {
    ...
    if (this._url) {
      if (this.sourceFunction !== fromArrayBuffer) {
        ...
        tiff = await fromArrayBuffer(this._url).catch((e: any) => {
          if (this.options.onError) {
            this.options.onError(e);
          } else {
  ...
  async _processTIFF(tiff: any) {
    this.tiff = tiff;
    await this.setBand(this.options.band).catch((e: any) => {
      console.error('this.setBand threw error', e);
    });
    if (!this.options.bounds) {
      const image = await this.tiff
        .getImage(this.options.image)
        .catch((e: any) => {
      ...
      if (window.Worker && this.options.useWorker) {
        const worker_src =
          'onmessage = function(e: any){let data = e.data.data; let noDataValue = e.data.noDataValue; let min = data.filter(val=> val !== noDataValue).reduce((a,b)=>Math.min(a,b)); let max = data.filter(val => val !== noDataValue).reduce((a,b)=>Math.max(a,b)); postMessage({min:min, max:max});}';
        ...
        this.min = this.raster.data[0].reduce((a: any, b: any) =>
          b === this.options.noDataValue ? a : Math.min(a, b)
        );
        this.max = this.raster.data[0].reduce((a: any, b: any) =>
          b == this.options.noDataValue ? a : Math.max(a, b)
        );
      }
    }
  },

  async setBand(band: any) {
    ...
    const a = this.options.transpValue
      ? data[this.options.alphaBand].map((v: any) => {
  ...
  getValueAtLatLng(lat: any, lng: any) {
    try {
      var x = Math.floor(
  ...
  setClip(clipLatLngs: any) {
    this.options.clip = clipLatLngs;
    this._reset();
  },

  _getPixelByLatLng(latLng: any) {
    var topLeft = this._map.latLngToLayerPoint(
      this._map.getBounds().getNorthWest()
    );
  ...
  createSubmask(size: any, args: any, clip: any) {
  ...
  transform(rasterImageData: any, args: any) {
   ...
   */
  getDescendantProp(obj: any, desc: any) {
...
export const leafletGeotiff = (url: any, options: any) => {
  const res = new LeafletGeotiff();
  res.initialize(url, options);
  return res;
};
